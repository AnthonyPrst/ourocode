...
class Objet(object):
    ...
    def _add_synthese_taux_travail(
        ...
        Args:
            lignes (Iterable): éléments de synthèse sous forme de:
                - dict avec clés (effort/Effort), (SN/situation_normale) et (SI/situation_incendie)
                - tuple/list (effort, taux_situation_normale, taux_situation_incendie)
            col_normale (str, optional): nom de la colonne pour la situation normale. Defaults to "Situation normale".
            col_incendie (str, optional): nom de la colonne pour la situation d'incendie. Defaults to "Situation d'incendie".
            arrondi (int|None, optional): nombre de décimales à conserver. None pour ne pas arrondir. Defaults to 3.

        Returns:
            pd.DataFrame: tableau de synthèse prêt à être exporté ou affiché.
        """
        ...
        for ligne in lignes:
            if isinstance(ligne, Mapping):
                ...
                effort, sn, si = (list(ligne) + [None] * 3)[:3]
            else:
    ...
    def synthese_taux_travail(self):
        """Retourne le DataFrame de synthèse partagé entre toutes les instances."""
